...
class UsuariosController extends BackendController {
     ...
     * Método para buscar
     */
    public function buscar($field='nombre', $value='none', $order='order.estado_usuario.asc', $page=1) {        
     ...
     * Método para listar
     */
    public function listar($order='order.estado_usuario.asc', $page='page.1') { 
        $page = (Filter::get($page, 'page') > 0) ? Filter::get($page, 'page') : 1;
        $usuario = new Usuario();
    ...
    public function editar($key) {        
        ...
        if(Input::hasPost('usuario')) {            
            ...
            if($persona) {
                if(Usuario::setUsuario('update', Input::post('usuario'), array('persona_id'=>$persona->id, 'repassword'=>Input::post('repassword'), 'id'=>$id, 'login'=>$usuario->login))) {
    ...
    public function estado($tipo, $key) {
        ...
        if($usuario->login == "admin") {
            Flash::info('¡No podés bloquear al Admin!');
            return Redirect::toAction('listar');
        }

        if($tipo == 'reactivar' && $usuario->estado_usuario == EstadoUsuario::COD_ACTIVO) {
            Flash::info('El usuario ya se encuentra activo.');
            return Redirect::toAction('listar');
        } else if($tipo == 'bloquear' && $usuario->estado_usuario == EstadoUsuario::COD_BLOQUEADO) {
